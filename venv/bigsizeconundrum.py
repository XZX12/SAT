...
def detect_two(array_size):
    global covered_list
    two_list = []
    non_cover_list = creat_non_covered_list(array_size)
    for pos in non_cover_list:
        if calc_qi(array_size,pos) == 1:
            temp_pos = valid_pos_connected(array_size,pos)
            if calc_qi(array_size,temp_pos) == 1:
                two_list.append((pos,temp_pos))
    return two_list
...
if __name__ == '__main__':
    ...
    while(not stop):
        ...
        for i in range(len(line_list)):
            if len(line_list[i])<=2 or len(line_list)>=8:
                stop = 0
                break
