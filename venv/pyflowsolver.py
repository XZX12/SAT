...
def show_solution(options, colors, decoded):
    ...
    for decoded_row in decoded:
        for (color, dir_type) in decoded_row:
            ...
            display_char_list.append(display_char)
...
def pyflow_solver_main(files):

    '''Main loop if module run as script.'''

    global display_char_list
    display_char_list = []

    is_result_exited = True
    ...
    options = parser.parse_args(files)
    ...
    for filename in options.filenames:
        ...
        if isinstance(sol, list):
            ...
            is_result_exited = False
            return is_result_exited
            result_char = 'u'
        else:
            result_char = 'f'
